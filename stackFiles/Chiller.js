...
const Chiler = () => {
  ...
  const calculateValues = () => {
    const refrigerationEffect =
      parseFloat(Mf) *
      1000 *
      parseFloat(Cp) *
      (parseFloat(Tin) - parseFloat(Tout));
    const refrigerationCapacity = refrigerationEffect / 3.51;
    const estimatedCompressorPower =
      parseFloat(W) * (parseFloat(nm) / 100) * (parseFloat(nt) / 100);
    const energyEfficiencyRatio =
      refrigerationEffect / estimatedCompressorPower;
    ...
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={styles.inputContainer}>
        <Text style={styles.label}>
          Chilled Water Temperature at Evaporator Inlet (Tin):
        </Text>
        <TextInput
          value={Tin}
          onChangeText={setTin}
      ...
      <View style={styles.inputContainer}>
        <Text style={styles.label}>
          Chilled Water Temperature at Evaporator Outlet (Tout):
        </Text>
        <TextInput
          value={Tout}
